...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride_conv, padding)
        self.conv.weight.data.mul_(weight_scale)
        
        conv_w = int((width - kernel_size + 2* padding)/stride_conv) + 1
        conv_h = int((height - kernel_size + 2* padding)/stride_conv) + 1
        
        self.fc1 = nn.Linear(int(width/2) * int(height/2) * num_filters, hidden_dim)
